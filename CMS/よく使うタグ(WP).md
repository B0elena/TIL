...
|意味|タグ|
|-|-|
|現在の投稿より新しい投稿へのリンクを表示 ※必ずループの中で使う|```<?php next_post_link(); ?>```|
|現在の投稿よりひとつ前の投稿へのリンクを表示 ※必ずループの中で使う必要|```<?php previous_post_link(); ?>```|
...
|ユーザーが持つメタデータから好きなものを表示|```<?php the_author_meta(); ?>```|
